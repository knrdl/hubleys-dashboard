...
const _particlesCache: Record<string, unknown> = {}

export async function getParticlesConfig(particles: string) {
  if (!_particlesCache.hasOwnProperty(particles)) _particlesCache[particles] = JSON.parse(await readFile(`particles/${path.basename(particles, '.json')}.json`))
