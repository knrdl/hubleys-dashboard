...
<svg viewBox="0 0 100 100" width="300" height="300" class="dark:text-neutral-300">
    <g transform="translate(50 50)">
        <circle cx="0" cy="0" r={circleRadius} fill="none" stroke="currentColor" stroke-width="5"
                class="text-gray-500/50"></circle>

        {#if isRunning}
            {#if mode === 'countdown'}
                <circle cx="0" cy="0" r={circleRadius} fill="none" stroke="currentColor" stroke-width="5"
                        stroke-dasharray="{fraction * 2 * circleRadius * Math.PI} 999"
                        stroke-linecap="round" style="transform: rotate(-90deg)"
        ...
        <text text-anchor="middle" fill="currentColor" dominant-baseline="middle" font-size="10"
              class="font-light tracking-wider">
            {text}
        </text>
    </g>
</svg>
