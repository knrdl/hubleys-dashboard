...
<div
	class="fixed top-0 left-0 bottom-0 right-0 z-[1000]"
	on:click|stopPropagation={() => dispatch('close')}
	on:keydown={keydown}
>
	<div
		class="absolute p-3 bg-gray-100/95 border border-gray-200 rounded-lg shadow-xl dark:bg-gray-900/95 dark:border-gray-700"
		bind:clientWidth={dialogWidth}
		bind:clientHeight={dialogHeight}
		style="left: {leftPos}px; top: {topPos}px"
		transition:scale={{ duration: 200 }}
	>
		<h1 class="text-center text-2xl mb-3 text-gray-900 dark:text-gray-100 select-none">
			{folderTitle}
		</h1>
		<div
		...
		>
			{#each menu || [] as item, idx}
				<div
					class:col-span-2={idx === menu.length - 1 && idx % 2 === 0}
